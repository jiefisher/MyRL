...
from .base_trainer import BaseTrainerMemoryManager
from .flat_tensors import BucketizedFlatTensors, FlatTensors
# from chatlearn.utils.logger import print_rank_0
...
class TrainerMemoryManagerV4(BaseTrainerMemoryManager):
    ...
    def offload_weights(self):
        ...
        if self._weights_offloaded:
            print_rank_0('Call offload_weights when already offloaded. Ignore it.')
            return
    ...
    def onload_weights(self):
        ...
        if not self._weights_offloaded:
            print_rank_0('Call onload_weights when already onloaded. Ignore it.')
            return
    ...
    def free_grad_buffers(self):
        ...
        if self._grad_buffers_freed:
            print_rank_0('Call free_grad_buffers when already freed. Ignore it.')
            return
    ...
    def build_grad_buffers(self):
        ...
        if not self._grad_buffers_freed:
            print_rank_0('Call build_grad_buffers when already built. Ignore it.')
            return
    ...
    def offload_main_weights(self):
        ...
        if self._main_weights_offloaded:
            print_rank_0('Call offload_main_weights when already offloaded. Ignore it.')
            return
