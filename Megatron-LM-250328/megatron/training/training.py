...
from .flat_tensors import FlatTensors, BucketizedFlatTensors
...
# from .base_trainer import create_trainer_memory_manager
...
def megatron_model(model):
    if isinstance(model, list):
        ...
        model = model
    return model
from .trainer_v4 import TrainerMemoryManagerV4
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    if args.check_weight_hash_across_dp_replicas_interval is not None:
        ...
        torch.distributed.barrier()
        print_rank_0(f">>> Weight hashes match after {iteration} iterations...")
    print_rank_0(torch.cuda.memory_allocated())
    _memory_manager = create_trainer_memory_manager(
    ...
    _memory_manager.free_grad_buffers()
    _memory_manager.offload_main_weights()
    _memory_manager.offload_optimizer_states()
    _memory_manager.offload_weights()
    torch.distributed.barrier()
    torch.cuda.synchronize()
    gc.collect()
    torch.cuda.empty_cache()
    ...
    torch.distributed.barrier()
    _memory_manager.onload_weights()

    _memory_manager.build_grad_buffers()

    _memory_manager.onload_main_weights()

    _memory_manager.onload_optimizer_states()
    torch.distributed.barrier()
    torch.cuda.synchronize()
    gc.collect()
    torch.cuda.empty_cache()
