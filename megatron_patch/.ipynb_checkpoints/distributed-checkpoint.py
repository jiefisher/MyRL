...
def broadcast_2d_tensor_within_mp(tensor, dtype=torch.float32):
    ...
    if torch.distributed.get_world_size(group) > 1:
        return broadcast_2d_tensor(tensor, mpu.get_model_parallel_src_rank(), group, dtype=dtype)
...
def broadcast_2d_tensor_within_pp(tensor, dtype=torch.float32, from_last: bool = True):
    ...
    if mpu.get_pipeline_model_parallel_world_size() > 1:
        return broadcast_2d_tensor(
            ...
            if from_last
            else mpu.get_pipeline_model_parallel_first_rank(),
...
class SyncTimer(NamedTimer):
    ...
    def sync_time(self, list_to_sync):
        ...
        torch.distributed.all_reduce(output_tensor, op=self.reduce_op, group=mpu.get_data_parallel_group())
...
def run_distributed_inference(inputs=None, infer_fn=None):
    ...
    if isinstance(outputs, tuple):
        ...
        rewards = rebalance_nd_tensor(rewards, group=mpu.get_data_parallel_group()).squeeze(1).cpu().numpy()
        values = rebalance_nd_tensor(values, group=mpu.get_data_parallel_group()).cpu().numpy()

        return rewards, values

    return rebalance_nd_tensor(outputs, group=mpu.get_data_parallel_group()).cpu().numpy()
