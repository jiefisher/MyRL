...
import megatron_patch.reward_score.instructions_registry as instructions_registry
...
def parse_and_eval(prompt,target):


#   inputs = read_prompt_list(_INPUT_DATA.value)
    json_input = json.loads(prompt)
    json_output = {"response":target}
    inputs = []
    return_dict = {}
    inputs.append(
    InputExample(key=None,
               instruction_id_list=json_input["instruction_id_list"],
               prompt=json_output["response"],
               kwargs=json_input["instruction_kwargs"]))
    return_dict[json_output["response"]] = json_output["response"]
...
#       _INPUT_RESPONSE_DATA.value)
    ...
    for func, output_file_name in [
      ...
      (test_instruction_following_loose, "eval_results_loose"),
        ...
        outputs = []
        for inp in inputs:
          outputs.append(func(inp, return_dict))
        follow_all_instructions = [o.follow_all_instructions for o in outputs]
        accuracy = sum(follow_all_instructions) / len(outputs)
#         logging.info("Accuracy: %f", accuracy)
#         print(accuracy)
#         print(follow_all_instructions)
    res = outputs[0].follow_instruction_list
